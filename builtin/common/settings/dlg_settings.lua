...
local function load()
	...
	local language_setting_info = get_setting_info("language")
	local language_labels = {
		[""] = fgettext_ne("(Use system language)"),
	}
	for _, locale in ipairs(language_setting_info.values) do
		if locale ~= "" then
			language_labels[locale] = ("%s [%s]"):format(core.get_locale_description(locale), locale)
		end
	end
	language_setting_info.option_labels = language_labels
